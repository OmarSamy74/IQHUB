...
export function FeatureCard({ icon: Icon, title, children, emoji, color, index = 0 }: FeatureCardProps) {
  return (
    <div className="relative group animate-fade-in" style={{animationDelay: `${index * 0.2}s`}}>
      ...
      <div className="relative">
        <div className="rounded-xl bg-white dark:bg-gray-800 px-8 py-12 shadow-lg hover:shadow-2xl transition-all duration-300 hover-lift border border-gray-100 dark:border-gray-700">
          ...
          <h5 className="mb-4 text-xl font-bold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{title}</h5>
          <p className="text-gray-600 dark:text-gray-300 leading-relaxed mb-6">{children}</p>
